...
from storm.common.decorators import Body, Get, HttpCode, Param, Post, Query
...
class UsersService:
    ...
    def get_users(self, q: str = None):
        ...
        users = [user.model_dump() for user in self.users if not q or q.lower() in user.name.lower()]
        return {"users": users}
